<template>
  <div style="width: calc(100% - 40px);height: 100%;transition: width 0s ease-in-out 0s;">
    <template v-if="isBucketList">
      ...
      <vxe-table
          ...
          :row-class-name="bucketTableRowClassName"
          :data="bucketListData"
      ...
      <el-card style="width: calc(100% - 50px);">
        ...
        <template #header>
          <div class="card-header">
            <div class="bucket-summary">
              <span class="iconfont icon-wenjiantong" style="font-size: 40px"></span>
              <div class="bucket-info">
                <h1>{{ bucketDetail.name }}</h1>
                <span>创建时间：<strong>{{ bucketDetail.createTime }}</strong></span>
                <span>权限：<strong>{{ bucketDetail.access }}</strong></span>
                <span>大小：<strong>{{ bucketDetail.size }}</strong></span>
                <span>文件数：<strong>{{ bucketDetail.objectNum }}</strong></span>
              </div>
            </div>
            <div class="bucket-action">
              <el-button size="large" :icon="RefreshRight" @click="setFileListTable()">刷新</el-button>
              <el-dropdown trigger="click" v-loading="fileUploadLoading">

                <el-button class="button" size="large" :icon="Upload" color="rgb(0,21,41)">上传</el-button>
                <template #dropdown>
                  <el-dropdown-menu style="width: 175px;">
                    <el-upload :auto-upload="false" multiple :show-file-list="false" :on-change="uploadFile">
                      <el-button size="large" style="width: 175px;" :icon="Upload">上传文件</el-button>
                    </el-upload>
                    <el-upload :auto-upload="false" multiple :show-file-list="false" :on-change="uploadDir">
                      <el-button size="large" style="width: 175px;" :icon="FolderAdd">上传文件夹
                      </el-button>
                    </el-upload>
                  </el-dropdown-menu>
                </template>
        ...
        <el-card>
          <div class="container">
            ...
            <el-button class="button" size="large" :icon="FolderAdd" @click="createDirDialogVisible=true">创建新路径
            </el-button>
            <el-dialog
            ...
            >
              ...
              <template #footer>
                <span class="dialog-footer">
                  ...
                  <el-button size="large" :disabled="newDir.length===0" type="primary"
                             @click="createDir">创建</el-button>
                </span>
              </template>
            </el-dialog>
              ...
              <el-card>
                <template #header>
                  <el-row :gutter="20" align="middle">
                    ...
                    <el-col :span="12">
                      <el-row>
                        <el-col :span="24"><span style="font-size: 25px">{{ fileDetail.objectName + " 版本" }}</span>
                        </el-col>
                      </el-row>
                      <el-row>
                        <el-col :span="8"><span style="font-size: 15px;">{{
                            fileDetail.versions.versionNum + "个版本"
                          }}</span></el-col>
                        <el-col :span="16"><span style="font-size: 15px;">{{ fileDetail.versions.totalSize }}</span>
                        </el-col>
                      </el-row>
                    </el-col>
...
<!--                    </el-col>-->
                ...
                <el-row class="version-list-item" v-for="(version,index) in fileDetail.versions.itemVOList"
                  ...
                  <el-col :span="24">
                    <el-row :gutter="20" align="middle">
                      ...
                      <el-col :span="2" style="font-weight: bold;font-size: 16px">v{{
                          fileDetail.versions.itemVOList.length - index
                        }}
                      </el-col>
                      <el-col :span="3">
                      ...
                      <el-col :span="2" :offset="13">
                        <el-tooltip
                            ...
                            placement="bottom"
                        >
                          <el-button type="info" size="small" circle :icon="Download" :disabled="version.deleteMaker"
...
<!--                            placement="bottom"-->
<!--                        >-->
<!--                          <el-button type="info" size="small" circle :icon="Refresh" :disabled="version.deleteMaker"-->
...
<!--                      </el-col>-->
                      <el-col :span="2">
                        <el-tooltip
                            ...
                            placement="bottom"
                        >
                          <el-button type="info" size="small" circle :icon="Delete" :disabled="version.deleteMaker"
          ...
          <el-card v-show="showFileDetail" style="width: 300px;">
            <el-row>
              <el-col :span="20">
                <div class="file-name"
                     style="font-size: 16px;height:30px;line-height:30px;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;">
            ...
            <el-card class="file-action">
              ...
              <el-divider/>
              <el-button :disabled="fileDetail.deleteMaker" text @click="downloadFile(null)">
                <el-icon>
                  <Download/>
                </el-icon>
                下载
              ...
              <el-divider/>
              <el-button :disabled="fileDetail.deleteMaker" text @click="tagsDialogVisible=true">
                <el-icon>
                  <CollectionTag/>
                </el-icon>
                标签
              ...
              >
                ...
                <span class="dialog-footer">
                  ...
                  <el-button size="large" :disabled="tagForm.tagKey.length===0||tagForm.tagLabel.length===0"
                             type="primary"
                             @click="createTag">保存</el-button>
                </span>
                </template>
              </el-dialog>
              <el-divider/>
              <el-button text :disabled="bucketDetail.versionControl==='OFF'"
                         @click="showFileVersionsList=!showFileVersionsList">
                <el-icon>
                  <Operation/>
                </el-icon>
...
<!--              </el-space>-->
            ...
            <template v-if="!fileDetail.deleteMaker">
              <el-descriptions
                  title="元数据"
                  direction="vertical"
                  :column="1"
...
import {
  ...
  Delete,
  Document,
  DocumentDelete, Download,
  Folder,
  FolderAdd, Operation, Refresh,
  RefreshRight,
  Search, Share,
  Upload
...
onMounted(async () => {
  ...
  const zonedDateTime = router.currentRoute.value.query.zonedDateTime;
  const access = router.currentRoute.value.query.access;
  const size = router.currentRoute.value.query.size;
  const objectNum = router.currentRoute.value.query.objectNum;
  const versionControl = router.currentRoute.value.query.versionControl;
  if (bucketName) {
    await goToFileListPage({
      row: {
        name: bucketName,
...
// 表格行类名
const bucketTableRowClassName = () => {
  return 'row-pointer'
}

const fileTableRowClassName = ({row}) => {
  const {deleteMaker} = row
  let className = "row-pointer"
  if (deleteMaker) className = className + " transparency"
  return className
}
...
const goToFileListPage = async ({row}) => {
  const {name: bucketName, createTime, access, size, objectNum, versionControl} = row
...
const setFileListTable = async (bucketName?: string, prefix?: string) => {
  fileListTableData.value = (await getFileListApi(bucketName ? bucketName : bucketDetail.value.name, prefix ? prefix : currentDir.value)).data
}
...
onUnmounted(() => {
  // 组件卸载时，将 loading 设置为 false，避免持续 loading
...
const uploadFile = async (uploadFile) => {
  fileUploadLoading.value = true
  const file = uploadFile.raw
  const res: any = await uploadFileApi(bucketDetail.value.name, file, currentDir.value)
  if (res.code === 200) {
    ElMessage({
      message: '上传成功',
      type: 'success',
 ...
 * 上传文件夹
 */
const uploadDir = async (uploadFile) => {
  fileUploadLoading.value = true
  const {raw} = uploadFile
  let prefix = ""
  const webkitRelativePath = raw.webkitRelativePath as string // 带文件路径的文件名
  const pathArr = webkitRelativePath.split("/").slice(0, -1)
  prefix = pathArr.join("/") + "/"
  // console.log("prefix", prefix)
  const formData = new FormData()
  formData.append("file", raw)
  const res: any = await uploadFileApi(bucketDetail.value.name, raw, currentDir.value + prefix)
  if (res.code === 200) {
    ElMessage({
      message: '上传成功',
      type: 'success',
...
const downloadFile = async (versionId: string | null) => {
  const res: any = await downloadFileApi(bucketDetail.value.name, currentDir.value, fileDetail.value.objectName, versionId)
  const url = res.data
...
const createTag = async () => {
  const res: any = await createTagApi(bucketDetail.value.name, fileDetail.value.objectName, tagForm.value.tagKey, tagForm.value.tagLabel)
  if (res.code === 200) {
...
const deleteTag = async (tagKey) => {
  ...
  fileDetail.value.tags.splice(fileDetail.value.tags.findIndex(tag => tag.tagKey === tagKey), 1)
}
...
const restoreFile = (fileName: string, versionId: string | null) => {
  ElMessageBox.confirm(
      `确定要恢复${fileName}文件的${versionId}版本?`,
      '恢复文件版本',
      {
...
const deleteFileVersion = (fileName: string, versionId: string | null) => {
  ElMessageBox.confirm(
      `确定要删除${fileName}文件的${versionId}版本?`,
      '删除文件版本',
      {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
...
::v-deep(.vxe-table .vxe-body--row.transparency) {
  opacity: 0.5;
}
